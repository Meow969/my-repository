...
function normalizeEchoResponse(payload, originalText = '') {
  const parsed = payload?.echoes || payload?.understanding || payload?.nextInsights
    ? payload
    : (payload?.data || extractJsonObject(payload?.output_text) || extractJsonObject(payload?.choices?.[0]?.message?.content) || {});
...
async function requestSiteEcho(text) {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      if (analysis.echoes.length) return { ...analysis, mode: data.mode || 'ai' };
    } catch (error) {
