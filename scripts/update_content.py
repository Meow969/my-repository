...
def title_specific_points(item: dict[str, Any]) -> list[str]:
    ...
    if entities or scenarios:
        ...
        if entities and scenarios:
            points.append(f"{entity_text}把AI能力放进{scenario_text}链路，重点不只是功能发布，而是用户决策和交易责任如何被重新分配。")
        elif entities:
            points.append(f"{entity_text}释放了新的AI购物相关动作，需要看它借用了哪些平台资产来改变用户路径。")
        else:
            points.append(f"这条资讯聚焦{scenario_text}，适合观察AI如何影响购物决策中的表达、比较、确认或履约环节。")
...
def make_specific_insight(item: dict[str, Any]) -> str:
    ...
    scene = "、".join(scenarios) or "AI购物"
    actor = "、".join(entities) if entities else "这类产品"
    title = str(item.get("title", ""))
    if contains_any(text, ["back-to-school", "返校", "school"]):
        return "用户需求：用户不是想多逛几个页面，而是想把一个阶段性任务一次配齐。｜产品机会：把返校、旅行、搬家、露营这类场景做成可编辑清单，自动补齐缺口、校验预算和配送时间。｜设计启发：推荐结果不要只给单品卡，可给“已满足/待确认/可替换”的任务清单视图。"
    if contains_any(text, ["gift", "perfect gift", "礼物", "送礼"]):
        return "用户需求：送礼场景里用户真正焦虑的是不失礼、不踩雷，而不是找到最多商品。｜产品机会：把关系、预算、场合、禁忌和到达时间做成送礼约束，先排除不合适选项再推荐。｜设计启发：卡片里展示“为什么适合这个人”和“可能不适合的原因”，比单纯相似推荐更有信任感。"
    if contains_any(title, ["message really from amazon", "真假", "诈骗", "fraud", "scam"]):
        return "用户需求：用户需要一个能即时判断消息真伪的可信入口，避免在钓鱼链接里完成支付或售后操作。｜产品机会：把安全校验做成购物助手的前置能力，覆盖订单通知、退款、客服和支付链接。｜设计启发：验证结果应直接连接官方订单状态和下一步操作，不让用户在真假页面之间来回判断。"
    if contains_any(text, ["trust", "risk", "permission", "sues", "mistake", "trusted", "privacy", "fraud", "安全", "授权", "信任", "风险", "隐私"]):
        return choose([
    ...
    if contains_any(text, ["product data", "trusted product data", "catalog", "metrics", "measurement"]):
        return choose([
            "用户需求：用户想知道推荐是否基于真实商品事实，而不是AI编出来的卖点。｜产品机会：把商品资料完整度、可引用证据、实时价格库存和转化指标做成同一套数据底座。｜设计启发：给商品增加“AI可读评分”，提示商家缺少哪些卖点、禁忌、适用人群或履约承诺。",
            "用户需求：用户希望AI推荐能说清楚“适合我”的依据，并且经得起价格、库存和评价核验。｜产品机会：商品库不只是搜索素材，而是导购的证据库和责任边界。｜设计启发：推荐卡应同时展示事实来源、更新时间、可替代项和不推荐条件。",
            "用户需求：当AI替用户比较商品时，用户需要看到稳定、可校验的事实，而不是营销话术。｜产品机会：把商品资料从文案资产升级为机器可读资产，服务推荐、比较、履约和归因。｜设计启发：商家后台可以提供“AI理解预览”，让商家看到商品会被如何解释和排序。",
        ])
    if contains_any(text, ["merchant", "merchants", "seller", "sellers", "retailer", "retailers", "marketplace", "marketplaces", "protect loyalty", "storefront", "商家", "卖家"]):
        return choose([
            f"用户需求：用户希望AI替自己筛掉不靠谱供给，商家则担心被AI入口重新分配流量。｜产品机会：为{actor}建设AI可读商品页、卖点证据、履约承诺和归因工具。｜设计启发：商家后台不只生成素材，还要提示“为什么没被推荐、缺什么证据、该补什么承诺”。",
            "用户需求：用户要的是可信候选，商家要的是被正确理解和公平呈现。｜产品机会：平台可把商品表达、服务承诺和评价证据变成商家侧的新运营资产。｜设计启发：新增“面向AI的店铺体检”，检查卖点、价格、库存、禁忌、售后规则是否足够清楚。",
            "用户需求：AI导购越强，用户越依赖它过滤供给，商家也越需要知道如何进入候选池。｜产品机会：把商家激励从填关键词转向补证据、补场景、补履约确定性。｜设计启发：推荐结果可标注商家承诺项，让好供给因为证据完整而被优先展示。",
        ])
    if contains_any(text, ["acquirer", "visa", "mastercard", "stripe", "payment", "checkout", "token", "支付", "收单", "结算"]):
        return choose([
            "用户需求：用户想省掉结算操作，但不愿把购买决定完全交出去。｜产品机会：把交易权限拆成建议、代填、锁价、代付、代买五级，让用户逐步授权。｜设计启发：支付确认页要同时展示预算、库存、优惠、到达时间和售后责任，形成可追责凭证。",
            "用户需求：用户接受AI帮忙买东西的前提，是价格变化、缺货、延迟、误购时能及时停下。｜产品机会：交易闭环的差异化不在更快付款，而在异常处理、撤回和责任说明。｜设计启发：给AI代购设置“变更阈值”，超过预算/时效/品牌条件时自动回到用户确认。",
            "用户需求：当AI进入支付，用户需要确认它是在执行自己的约束，而不是替平台做转化。｜产品机会：把推荐理由和支付凭证绑定，形成从建议到成交的责任链。｜设计启发：订单页可回放AI依据哪些条件做了选择、哪些条件触发人工确认。",
        ])
    if contains_any(text, ["app in chatgpt", "inside chatgpt", "instant checkout", "openai", "chatgpt", "perplexity", "paypal"]):
        return choose([
            "用户需求：用户从外部AI带着半成型答案进站时，不想重新搜索和重复解释。｜产品机会：做“意图交接层”，把预算、场景、禁忌、候选商品转成站内可操作任务。｜设计启发：落地页先还原用户问题和已有候选，再补价格、库存、评价和优惠证据。",
            "用户需求：用户希望AI答案能自然进入购买，而不是停在建议。｜产品机会：站内导购要承接外部AI上下文，继续完成取舍、核价和确认。｜设计启发：在商品页提供“继续刚才的比较”入口，减少从答案页到交易页的断裂。",
            "用户需求：用户真正要的是连续决策流，而不是每个平台各问一遍。｜产品机会：外部AI入口会重分配流量，平台要争夺的是上下文承接和交易完成能力。｜设计启发：把AI答案中的需求条件转成可编辑筛选器，让用户一眼校正。",
        ])
    if contains_any(text, ["memory", "personalization", "personalized", "preference", "context", "habit", "记忆", "偏好", "个性化", "上下文"]):
        return choose([
            "用户需求：用户希望AI记住偏好，但也要能看见、修改和删除它。｜产品机会：把记忆产品化成“我的购买规则”，沉淀预算、尺码、品牌黑白名单、复购周期和场景偏好。｜设计启发：每次推荐都标出命中的记忆项，并允许用户一键纠正。",
            "用户需求：用户不想反复说明常买规格、忌口、尺码和预算。｜产品机会：先在复购、耗材、尺码等低争议场景建立可控记忆，再扩展到高客单决策。｜设计启发：把记忆做成规则卡，而不是不可解释的画像标签。",
            "用户需求：用户需要个性化带来省心，而不是被黑箱推荐困住。｜产品机会：让偏好记忆同时服务推荐和解释，帮助用户理解“为什么这次这样推荐”。｜设计启发：提供“本次不适用”按钮，避免一次性偏好污染长期记忆。",
        ])
    if contains_any(text, ["voice", "image", "camera", "lens", "multimodal", "视觉", "语音", "图片", "拍照", "多模态", "识图"]):
        return choose([
            "用户需求：很多购物需求说不清，但能被图片、语音或场景触发。｜产品机会：多模态入口要把风格、用途、限制条件转成候选商品和排除理由。｜设计启发：识别后先展示AI理解到的场景标签，让用户确认再进入推荐。",
            "用户需求：用户想用一张图或一句话表达灵感，而不是学习复杂筛选词。｜产品机会：把多模态识别结果直接连接相似款、搭配、替代品和风险提示。｜设计启发：结果页不仅说“看见了什么”，还要说“因此排除了什么”。",
            "用户需求：用户在非标品里更难描述审美、风格和适配边界。｜产品机会：图片/语音入口可以把模糊灵感翻译成可比较商品集合。｜设计启发：把识别出的颜色、版型、材质、场景做成可编辑条件 chips。",
        ])
    if contains_any(text, ["operator", "computer use", "browser", "agent", "tasks", "workflow", "智能体", "浏览器", "任务", "工作流"]):
        return choose([
            "用户需求：用户愿意把重复劳动交给AI，但关键决策仍想自己掌控。｜产品机会：把导购拆成可接管的小任务：查证、比价、凑单、补货、售后提醒。｜设计启发：每个任务都要有暂停点、任务日志和人工确认，而不是直接全自动购买。",
            "用户需求：用户需要AI帮忙跑腿，但不能替自己做不可逆决定。｜产品机会：智能体先进入可验证动作，例如抓参数、做对比、查优惠、核库存。｜设计启发：用任务进度条展示AI已做/待确认/失败回退，让代劳变得可监督。",
            "用户需求：跨页面购物最麻烦的是信息分散和重复核对。｜产品机会：浏览器式智能体可把跨站查价、评价归纳和候选筛除做成连续流程。｜设计启发：保留“看过哪些页面、为什么筛掉”的决策日志，降低黑箱感。",
        ])
    if contains_any(text, ["conversion", "spending", "traffic", "sales", "%", "转化", "流量"]):
        return choose([
            "用户需求：用户要更快做出放心选择，平台要证明AI不是只带来噱头流量。｜产品机会：把AI效果拆到需求表达、候选点击、加购、客单、复购和售后成本。｜设计启发：每个导购功能上线时绑定一个链路指标，而不是只看总转化。",
            "用户需求：用户不关心AI多聪明，只关心少花时间、少买错。｜产品机会：增长实验应验证AI减少了哪一步成本：搜索、比较、决策、下单还是售后。｜设计启发：推荐卡旁展示“节省了什么动作”，帮助团队判断功能价值。",
            "用户需求：用户希望推荐结果更接近自己的真实意图。｜产品机会：把转化增长拆成可解释归因，区分短期流量、推荐质量和交易闭环贡献。｜设计启发：建立漏斗看板，分别追踪AI改写、证据点击、加入购物车和确认支付。",
        ])
    if contains_any(text, ["tested", "i let", "which worked best", "perfect gift", "实测", "测评"]):
        return choose([
            "用户需求：真实用户要的是可靠结果，不是听起来聪明的回答。｜产品机会：把测评里的失败样例沉淀成导购验收集，覆盖约束理解、核价、库存、替代方案和不推荐理由。｜设计启发：每条推荐都要能通过“现在能买吗、为什么选它、哪里可能不适合”的检查。",
            "用户需求：用户很快会用失败体验判断AI是否值得托付。｜产品机会：把公开测评变成产品测试用例，比只看发布稿更能发现短板。｜设计启发：在推荐结果里主动暴露不确定项，避免“看似合理但不可买”。",
            "用户需求：用户需要AI能处理真实约束，而不是只会回答理想题。｜产品机会：测评内容可反向定义P0能力：理解预算、场景、偏好、库存和配送。｜设计启发：把“被排除商品及原因”作为默认输出，减少用户二次追问。",
        ])
    if is_visual_try_on_context(text):
        return choose([
            "用户需求：买服饰、美妆、鞋履时，用户真正想判断的是“适不适合我”。｜产品机会：视觉体验要进入决策证据链，把尺码、风格、场景和退货风险用于推荐排序。｜设计启发：试穿图旁边给出“合适点/风险点/替代款”，而不是只展示一张好看的图。",
            "用户需求：非标品决策难在想象成本和后悔风险。｜产品机会：AI试穿应从营销玩法升级为购买前验证工具。｜设计启发：把上身效果、搭配场景、尺码建议和退货概率并排展示，帮助用户做取舍。",
            "用户需求：用户需要把灵感转成适合自己的商品，而不是看更多模特图。｜产品机会：视觉导购可以把风格识别、相似款、搭配和适配模拟串起来。｜设计启发：允许用户点选“不像我/太正式/预算过高”等反馈，实时收窄候选。",
        ])
    if contains_any(text, ["grocery", "quick-commerce", "instant", "闪购", "外卖", "买菜", "本地生活"]):
        return choose([
            "用户需求：本地生活里用户要的是“现在附近、预算内、能准时到”，不是长篇商品科普。｜产品机会：从补货、点餐、买菜、凑单等高频低风险任务训练AI授权习惯。｜设计启发：推荐卡优先显示可达时间、库存、替代方案和失败补偿。",
            "用户需求：即时零售的核心是确定性，用户不想在缺货、排队、配送延迟中反复决策。｜产品机会：AI可以承担实时约束匹配，把位置、时段、库存、优惠和服务承诺一起算。｜设计启发：下单前展示“最稳方案/最快方案/最省方案”，让用户按目标切换。",
            "用户需求：日常小额消费希望少想一步，但仍要能控制偏好和预算。｜产品机会：把AI导购先做成复购和临时任务助手，再逐步进入复杂购物。｜设计启发：沉淀常买清单与替换规则，例如缺货时可替品牌、不可替规格。",
        ])
    if contains_any(text, ["instacart"]):
        return "用户需求：买菜用户往往不是要找单个SKU，而是想把一餐、一周或某个场景的清单快速补齐。｜产品机会：把闲聊意图转成可执行购物篮，同时处理库存、替代品、配送窗口和凑单。｜设计启发：用“清单完成度”呈现AI贡献，标出已满足、需用户确认、缺货替代和预算变化。"
    if contains_any(text, ["styling", "style", "fashion", "apparel", "outfit", "穿搭", "服饰", "造型"]):
        return "用户需求：审美型购物里用户更关心风格是否适合自己，而不是参数是否最优。｜产品机会：把AI导购做成风格翻译器，连接灵感、相似款、搭配和场景适配。｜设计启发：输出“适合什么场合/不适合哪里/可替代风格”，帮助用户减少后悔成本。"
    if entities or scenarios:
        return f"用户需求：用户希望AI在{scene}里把模糊需求翻译成可执行选择，并减少自己反复查证的成本。｜产品机会：观察{actor}调用了哪些资产来完成任务，例如商品库、评价、库存、优惠、订单或履约能力。｜设计启发：把体验拆成入口、意图、证据、动作、兜底五个节点，找出可迁移到自身产品的最小设计。"
    return ""
...
def disambiguate_duplicate_core_points(articles: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in articles:
        ...
        if counts.get(sig, 0) > 1:
            ...
            differentiator = f"这条资料的可用点在于补充{scenario}链路里的一个观察样本，可继续追踪真实入口、用户反馈和交易推进深度。"
...
def make_insight(item: dict[str, Any], tags: list[str]) -> str:
    ...
    if specific:
        return specific
    if "market_signal" in angles:
        return "用户需求：用户不会因为AI概念买单，只会为更省心、更确定的购买体验买单。｜产品机会：把赛道热度转成竞品优先级和产品验证题，而不是直接当成功结论。｜设计启发：继续追官方发布、体验截图和用户反馈，确认它影响的是入口、转化、复购还是履约成本。"
    if "consumer_benchmark" in angles:
        return "用户需求：用户希望AI在真实购物约束下仍然可靠，而不是只在演示里表现聪明。｜产品机会：把测评内容转成验收标准，覆盖理解约束、核价、库存、配送和替代方案。｜设计启发：推荐结果默认说明“为什么选它、为什么不选其他、现在是否可买”。"
    if "ai_search_commerce" in angles:
        return "用户需求：用户从AI答案页进入购物时，希望上下文被保留，不想从关键词搜索重来。｜产品机会：打造AI入口后的承接层，把问题、候选、预算和约束转成站内任务。｜设计启发：落地页先展示AI已理解的条件，再补证据、价格、库存和购买确认。"
    if "agentic_checkout" in angles:
        return "用户需求：用户想让AI省掉交易操作，但不能失去确认权和追责权。｜产品机会：把逛、选、比、确认、支付、售后做成连续责任链。｜设计启发：每一步都提供解释、撤回和兜底，越接近支付越要显性确认。"
    if "visual_try_on" in angles:
        return "用户需求：非标品购物的核心问题是“适不适合我”，不是缺少更多图片。｜产品机会：把视觉能力变成尺码、风格、场景和退货风险的决策证据。｜设计启发：推荐卡展示合适点、风险点和替代款，让用户更快判断是否继续。"
    if "product_data" in angles:
        return "用户需求：用户需要AI推荐能被事实验证，而不是只听起来合理。｜产品机会：建设机器可读的商品事实层，覆盖卖点、适用人群、禁忌、库存、价格和履约承诺。｜设计启发：把证据完整度做成商品和商家的基础评分。"
    if "merchant_tools" in angles:
        return "用户需求：用户需要高质量供给被准确推荐，商家需要知道如何被AI理解。｜产品机会：同时设计商家端体验，把商品表达、素材、投放和服务承诺转成AI能调用的资产。｜设计启发：商家后台增加“被AI推荐的缺口诊断”和证据补全建议。"
    if "local_life" in angles:
        return "用户需求：本地生活用户最在意当下确定性：附近、预算内、能准时送达或到店。｜产品机会：先从高频、低风险、强时效任务培养AI购物习惯。｜设计启发：用最快、最稳、最省三个方案承接不同决策目标。"
    if "platform_assistant" in angles:
        return "用户需求：用户期待AI少做决策杂活，而不是多一个聊天入口。｜产品机会：竞品拆解要看它具体接管了搜索、比较、加购、支付还是售后。｜设计启发：用入口、意图、资产、动作、回退五项表格复盘，不只记录“上线AI助手”。"
    if "discovery_decision" in angles:
        return "用户需求：用户常常只有模糊目标，需要AI帮他把灵感变成可比较选择。｜产品机会：导购价值不在给更多商品，而在组织候选集合和取舍理由。｜设计启发：输出场景匹配、关键差异和反例提醒，减少用户反复追问。"
    if "trust_risk" in angles:
        return "用户需求：用户需要知道AI推荐从哪里来、是否可靠、出错后怎么办。｜产品机会：把信任设计前置到推荐过程，而不是只做事后客服。｜设计启发：展示证据来源、排序理由、可撤回授权和售后责任。"
    if any(word in lower for word in ["sparky", "alexa for shopping", "rufus", "ai shopping assistant", "ai-powered shopping"]):
        return f"用户需求：用户希望购物助手替自己少查少比，但仍能解释选择。｜产品机会：{subject}可用来观察平台级商品、库存、评价和订单资产如何转成AI优势。｜设计启发：拆解四个阈值：是否记住偏好、解释排序、处理价格提醒、进入低风险自动购买。"
    if any(word in title for word in ["问小团", "小美"]):
        return f"用户需求：本地生活用户要的是今晚、附近、预算内的确定方案。｜产品机会：{subject}说明导购可以从服务供给实时性切入，而不只是问答。｜设计启发：把排队、配送、营业时间、优惠和替代店铺放进同一张决策卡。"
    if any(word in title for word in ["千问", "淘宝", "天猫", "AI万能搜"]):
        return f"用户需求：用户希望AI直接理解自己的购物历史和当前任务，而不是重新问一遍。｜产品机会：{subject}提醒平台要把AI嵌进历史订单、收藏、购物车、优惠、售后和商家工具。｜设计启发：让AI输出可执行清单，用户确认后直接进入加购、凑单或售后动作。"
    if "虚拟试穿" in tags:
        return f"用户需求：用户想先判断上身、上脸或上脚效果，降低买错和退货风险。｜产品机会：{subject}可启发非标品导购把预体验前置。｜设计启发：沉淀尺码、风格、场景偏好，并把不适合原因展示出来。"
    if "竞品案例" in tags:
        return "用户需求：用户不是要更多AI入口，而是要更短、更确定的购买路径。｜产品机会：竞品案例要拆成入口、意图、商品/内容资产、交易推进和失败回退。｜设计启发：把每个竞品功能还原成用户少做了哪一步，而不是只记录功能名。"
    if "交易闭环" in tags or "商品库" in tags:
        return "用户需求：用户需要推荐后能放心下单，不想自己再核价、看库存、查售后。｜产品机会：对话导购要接入可信商品资料、实时价格库存、优惠和售后规则。｜设计启发：把“可买性核验”放在推荐前后，避免只种草不负责。"
    if "技术架构" in tags:
        return "用户需求：用户需要稳定完成购物任务，而不是和模型反复试探。｜产品机会：技术架构要服务可控流程，把导购拆成需求澄清、候选生成、证据比较、风险提示、下单确认。｜设计启发：每个模块都有输入、输出和失败兜底，降低幻觉和误购。"
    if "即时零售" in tags:
        return "用户需求：日常消费想少想一步，但对送达时间和替代品很敏感。｜产品机会：先从复购、买菜、日用品等低风险场景建立偏好记忆。｜设计启发：设计缺货替换、预算上限和配送时效确认，比复杂大件更容易形成使用习惯。"
    if "GEO" in tags:
        return f"用户需求：用户会越来越依赖AI答案选择商品，商家需要被正确理解和引用。｜产品机会：{subject}指向AI可见性运营，让卖点、场景、证据和履约承诺机器可读。｜设计启发：给商家提供答案页预览和缺口诊断，告诉它哪些信息影响被推荐。"
    if "Agentic Commerce" in tags:
        return f"用户需求：用户希望AI替自己推进购物任务，同时保留关键决策权。｜产品机会：{subject}的重点是重分配发现、比较、确认和支付之间的责任。｜设计启发：把AI可代劳动作和必须人工确认动作分层展示。"
    scenario = "、".join(detected_labels(article_context({**item, "tags": []}), SCENARIO_RULES, 2)) or "购物决策链路"
    tag_focus = "、".join(tags[:2]) or "AI导购"
    return f"用户需求：用户希望在{scenario}里更快形成判断并减少反复确认。｜产品机会：围绕{tag_focus}设计一个小实验，验证AI是否降低表达成本、比较成本或交易风险。｜设计启发：记录用户在哪一步追问、放弃或人工接管，把资讯沉淀成可复用假设。"
